...
func TestHealthzEndpoint(t *testing.T) {
	cfg := DefaultConfig()
	srv := New(store.NewInMemoryInstrumentStore(), store.NewInMemoryOrderStore(), store.NewInMemoryTradeStore(), store.NewInMemoryPositionStore(), nil, store.NewInMemoryCorporateActionStore(), store.NewInMemoryEntitlementStore(), nil, nil, nil, nil, cfg)
	srv.SetReady()
