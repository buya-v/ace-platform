...
func newTestServerWithCA(t *testing.T, s caStores) *httptest.Server {
	...
	srv := New(s.instrument, s.order, s.trade, s.position, nil,
		s.corporateAct, s.entitlement, store.NewInMemoryMarketStore(), store.NewInMemorySegmentStore(), store.NewInMemoryCircuitBreakerStore(), store.NewInMemoryFirmStore(), store.NewInMemoryParticipantStore(), nil, nil, nil, nil, nil, nil, nil, nil, nil, nil,
		nil, // locateStore
		nil, // rfqStore
		nil, // giveUpStore
