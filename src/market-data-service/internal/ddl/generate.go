...
func GenerateTradesHypertable() string {
	return `CREATE TABLE IF NOT EXISTS market_data.trades (
...
SELECT create_hypertable('market_data.trades', 'executed_at',
    chunk_time_interval => INTERVAL '1 day',
    if_not_exists => TRUE
);

CREATE INDEX IF NOT EXISTS idx_trades_instrument_seq
    ON market_data.trades (instrument_id, sequence_number DESC);`
}
...
func GenerateContinuousAggregate(interval string, bucketWidth string, sourceTable string) string {
	viewName := fmt.Sprintf("market_data.candles_%s", interval)
...
func GenerateAllAggregates() string {
	...
	ddl += GenerateContinuousAggregate("1m", "1 minute", "market_data.trades") + "\n\n"

	// 5-minute from 1m
	ddl += "-- 5-minute candles (from 1m)\n"
	ddl += GenerateContinuousAggregate("5m", "5 minutes", "market_data.candles_1m") + "\n\n"

	// 15-minute from 1m
	ddl += "-- 15-minute candles (from 1m)\n"
	ddl += GenerateContinuousAggregate("15m", "15 minutes", "market_data.candles_1m") + "\n\n"

	// 1-hour from 1m
	ddl += "-- 1-hour candles (from 1m)\n"
	ddl += GenerateContinuousAggregate("1h", "1 hour", "market_data.candles_1m") + "\n\n"

	// 4-hour from 1h
	ddl += "-- 4-hour candles (from 1h)\n"
	ddl += GenerateContinuousAggregate("4h", "4 hours", "market_data.candles_1h") + "\n\n"

	// 1-day from 1h
	ddl += "-- 1-day candles (from 1h)\n"
	ddl += GenerateContinuousAggregate("1d", "1 day", "market_data.candles_1h") + "\n"
...
SELECT add_retention_policy('market_data.trades', INTERVAL '90 days', if_not_exists => TRUE);
SELECT add_retention_policy('market_data.candles_1m', INTERVAL '1 year', if_not_exists => TRUE);
SELECT add_retention_policy('market_data.candles_5m', INTERVAL '1 year', if_not_exists => TRUE);
SELECT add_retention_policy('market_data.candles_15m', INTERVAL '1 year', if_not_exists => TRUE);
SELECT add_retention_policy('market_data.candles_1h', INTERVAL '2 years', if_not_exists => TRUE);
