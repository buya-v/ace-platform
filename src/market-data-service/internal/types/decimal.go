...
func (d Decimal) GreaterThanOrEqual(other Decimal) bool { return d.value >= other.value }
func (d Decimal) LessThanOrEqual(other Decimal) bool  { return d.value <= other.value }

func (d Decimal) Add(other Decimal) Decimal { return Decimal{value: d.value + other.value} }
func (d Decimal) Sub(other Decimal) Decimal { return Decimal{value: d.value - other.value} }

func (d Decimal) MulUint64(qty uint64) Decimal {
	return Decimal{value: d.value * int64(qty)}
}

// DivInt divides the decimal by an integer. Used for VWAP = turnover / volume.
func (d Decimal) DivInt(divisor int64) Decimal {
	if divisor == 0 {
		return DecimalZero()
	}
	return Decimal{value: d.value / divisor}
}
