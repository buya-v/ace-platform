...
// the fail-fast behaviour of NewProducerFromEnv / NewConsumerFromEnv. Outside of
// tests, an unset/empty value triggers log.Fatal rather than a silent in-process
// fallback (R007).
...
// TestNewInProcessProducer verifies the explicit test-only constructor returns a
// usable in-process channel producer. This adapter is reachable from production
// code only via NewProducerFromEnv under testing.Testing().
func TestNewInProcessProducer(t *testing.T) {
	p := newInProcessProducer()
	if p == nil {
		t.Fatal("newInProcessProducer returned nil")
	}
...
// usable in-process channel consumer.
