...
const startTrading: Section = {
  ...
  steps: [
    ...
    {
      id: 'trade-6',
      title: 'GOV: Submit Buy Order',
      description: 'Trader 1 buys GOV Mining shares at 2500 MNT',
      method: 'POST',
      url: '/api/v1/securities/orders',
      headers: (state) => tenantHeader(state, 'trader1'),
      body: (state) => ({
        instrument_id: state.gov_id || 'GOV',
        side: 'BUY',
        order_type: 'LIMIT',
        quantity: 200,
        price: 2500,
    ...
    {
      id: 'trade-7',
      title: 'GOV: Submit Matching Sell Order',
      description: 'Trader 2 sells GOV at 2500 — instant match',
      method: 'POST',
      url: '/api/v1/securities/orders',
      headers: (state) => tenantHeader(state, 'trader2'),
      body: (state) => ({
...
const postTrade: Section = {
  ...
  steps: [
    ...
    },
    {
      ...
      description: 'Check positions — APU and GOV should both have position records',
      method: 'GET',
    ...
    },
    {
      ...
      description: 'Review matched trades from auction and continuous sessions',
      method: 'GET',
      url: '/api/v1/securities/trades',
      headers: (state) => tenantHeader(state, 'admin'),
      validateResponse: okValidator,
    },
    {
      id: 'post-7',
      title: '🔍 Flag Large Trade Alert',
      description: 'Surveillance: admin flags a large trade for review',
      method: 'POST',
      url: '/api/v1/securities/surveillance/alerts',
      headers: (state) => tenantHeader(state, 'admin'),
      body: (state) => ({
        instrument_id: state.apu_id || 'APU',
        alert_type: 'LARGE_TRADE',
        status: 'OPEN',
        message: `Large trade alert: 100 shares of APU filled at 850 MNT (value: 85,000 MNT)`,
      }),
      validateResponse: okValidator,
      extractState: (body) => {
        const b = body as Record<string, unknown>;
        return { alert_id_1: b.id || b.ID };
      },
    },
    {
      id: 'post-8',
      title: '🔍 Flag Price Spike Alert',
      description: 'Surveillance: price moved from 845 to 860 (+1.8%) during session',
      method: 'POST',
      url: '/api/v1/securities/surveillance/alerts',
      headers: (state) => tenantHeader(state, 'admin'),
      body: (state) => ({
        ...
        status: 'OPEN',
        message: 'Price spike detected: APU moved from 845 to 860 MNT (+1.8%) in one session',
      }),
      validateResponse: okValidator,
    },
    {
      ...
      description: 'Dashboard should show 2 open alerts',
      method: 'GET',
    ...
    {
      id: 'post-10',
      title: '✅ Resolve Large Trade Alert',
      description: 'Admin reviews and resolves the large trade alert',
      method: 'PUT',
      url: (state) => `/api/v1/securities/surveillance/alerts/${state.alert_id_1 || 'unknown'}/resolve`,
      headers: (state) => tenantHeader(state, 'admin'),
      body: () => ({ resolved_by: 'admin@garudax.mn' }),
      validateResponse: okValidator,
    },
  ],
};
