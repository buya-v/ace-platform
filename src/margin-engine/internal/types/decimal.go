...
// Matches the clearing-engine and matching-engine Decimal type for cross-service compatibility.
type Decimal struct {
	value int64
}
...
func (d Decimal) MulInt64(qty int64) Decimal {
	return Decimal{value: d.value * qty}
}
...
// MulDecimal multiplies two decimals. Since both are scaled by 10000,
// the product must be divided by 10000 to maintain scale.
func (d Decimal) MulDecimal(other Decimal) Decimal {
	return Decimal{value: (d.value * other.value) / decimalScale}
}

// DivInt64 divides by an integer.
func (d Decimal) DivInt64(divisor int64) Decimal {
	if divisor == 0 {
		return DecimalZero()
	}
